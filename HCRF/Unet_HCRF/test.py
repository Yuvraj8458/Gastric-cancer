...
def test(m_class, train_num):
    ...
    test_path = file_path + "/" + str(i) + "/test"
    model_path = test_path + "/model/" + type
    original_val_path=r"E:/20190429-Gastric-Carcinoma-Cancer-Subset-Division/test/unet/patch/1/test"
    if predict:
        start2 = time.perf_counter()
        model = load_model(model_path + "/unet_membrane" + str(n) + ".hdf5")
        testGene = testGenerator(original_val_path + "/" + "original", num_image=test_num, target_size=data_size)
